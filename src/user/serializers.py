...
class UserSerializer(serializers.ModelSerializer):
    ...
    gigs = serializers.SerializerMethodField(read_only=True)
    payments = serializers.SerializerMethodField(read_only=True)

    def get_payments(self, obj):
        """
        Adds payments to user
        """
        results = Payment.objects.filter(user=obj)
        return PaymentSerializer(results, many=True).data


    def get_gigs(self, obj):
        """
        Adds gigs to the user/me response.
        """
        results = Gig.objects.filter(user=obj)
        return GigSerializer(results, many=True).data

    
    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'gigs',
                  'mobile_number', 'date_joined', 'password', 'is_superuser', 'is_staff', 'completed_onboarding', 'completed_tour', 'associated_hs_deal_id', 'payments')
        read_only_fields = ['associated_hs_deal_id']
...
class FeedbackSerializer(serializers.ModelSerializer):
    """
    This class validates fields.
    """
    feedback_type = serializers.CharField(write_only=True)
    feedback_comments = serializers.CharField(write_only=True)

    class Meta:
        model = User
        fields = ("feedback_type", "feedback_comments", )

class RequestAppSerializer(serializers.ModelSerializer):
    """
    This class validates fields.
    """
    requested_connection = serializers.CharField(write_only=True)

    class Meta:
        model = User
        fields = ("requested_connection", )
