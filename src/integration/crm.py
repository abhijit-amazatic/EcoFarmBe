...
def post_leads_on_slack(record):
    ...
    """
    msg = """New lead is added via connect page with the details as:\n- First Name:%s\n- Last Name:%s\n- Company Name:%s\n -Title:%s\n- Vendor Category:%s\n- Heard From:%s\n- Phone:%s\n- Message:%s\n- Email:%s """ %(record.get("first_name"),record.get("last_name"),record.get("company_name"),record.get("title"),','.join(record.get("vendor_category")),record.get("heard_from"),record.get("phone"),record.get("message"),record.get("email"))
